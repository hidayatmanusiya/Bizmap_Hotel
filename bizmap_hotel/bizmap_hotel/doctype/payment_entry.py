...
def before_submit(doc,method):
    ...
    room_folio=frappe.db.get_value("Room Folio HMS",{"name":doc.room_folio_reference},"name")
    if room_folio:
...
@frappe.whitelist() 	
def payment_entry_in_collection_pymt_room_folio(doc):
    doc =json.loads(doc)
    frappe.reload_doctype("Room Folio HMS")
    room_folio=frappe.db.get_value("Room Folio HMS",{"name":doc.get("room_folio_reference")},"name")
    if room_folio:
       ref_name= frappe.db.sql(f""" select reference_name from `tabPayment Entry Reference` where parent="{doc.get("name")}"  """,as_dict=1)
       for j in ref_name:
            paymet_entry=frappe.db.sql(f""" select a.name,a.paid_amount,a.posting_date from `tabPayment Entry` as a inner join `tabPayment Entry Reference` as p on p.parent=a.name where p.reference_name="{j.reference_name}" """,as_dict=1)
            room_folio_payment=frappe.get_doc('Room Folio HMS',room_folio)
            room_folio_payment.collected_payment=[]
            for i in paymet_entry:
                payment_entry_child=room_folio_payment.append('collected_payment',{})
                ...
                payment_entry_child.amount=i.paid_amount
                room_folio_payment.run_method('submit')
                room_folio_payment.reload() #doc.reload()
